...
#define PROGRAM "ntSketch"

static const char VERSION_MESSAGE[] =
    PROGRAM " 1.1.0 \n"
    "Written by Hamid Mohamadi.\n"
    "Alterations intended to approximate cosine distances written by Kevin Wasiluk.\n"
...
static const char USAGE_MESSAGE[] =
    ...
    "  -S, --sketch  saves .ntsketch files for given input files\n"
    "  -d, --pairwise-distances  provides file of pairwise cosine distances for given files"
...
bool samH=true;
bool sketch=false;
bool pairwise_d=false;
}

static const char shortopts[] = "t:s:r:k:c:l:p:f:o:Sd";
...
static const struct option longopts[] = {
    ...
    {"sketch", no_argument, NULL, 'S'}, //writes sketches to present working directory
    {"pairwise-distances", no_argument,NULL,'d'}, //just writes pairwise-distances, but doesn't save sketches
    { NULL, 0, NULL, 0 }
};


//double is probably sufficient here... but might be safer to switch to long double.
double sumOfSquares(double fMean[]){
    double ret = 0;
    for(size_t i = 1; i < opt::covMax; i++){
        ret += (double)(fMean[i]*i*i);
    }
    return ret;
}
...
inline void ntReadwz(const string &seq, const std::vector<unsigned> &kList, uint16_t *t_Counter, size_t totKmer[], unsigned w){
    ntHashIterator itr(seq, kList[0]);
    while(itr != itr.end()){
        ntComp((*itr), t_Counter+w*opt::nSamp*opt::rBuck);
        ++itr;
        ++totKmer[0];
    }
}
...
void getEfq(std::ifstream &in, const std::vector<unsigned> &kList, uint16_t *t_Counter, size_t totKmer[], unsigned w) {
    bool good = true;
    if(w == 0){
        ...
        for(string seq, hseq; good;) {
            ...
            good = static_cast<bool>(getline(in, hseq));
            if(good){
                ntReadwz(seq, kList, t_Counter, totKmer,w);
            }
            good = static_cast<bool>(getline(in, hseq));
        }       
    }
...
void getEfa(std::ifstream &in, const std::vector<unsigned> &kList, uint16_t *t_Counter, size_t totKmer[], unsigned w) {
    bool good = true;
    if(w ==0){
        for(string seq, hseq; good;) {
            string line;
        ...
        for(string seq, hseq; good;) {
            ...
            ntReadwz(line, kList, t_Counter, totKmer,w);
        }  
    }

}

void getEsm(std::ifstream &in, const std::vector<unsigned> &kList, const std::string &samSeq, uint16_t *t_Counter, size_t totKmer[], unsigned w) {
...
void readSketch(std::ifstream &in, uint16_t *t_Counter){ 
    unsigned zeroth;
    unsigned first;
    size_t second; 
    in >> zeroth;
    in >> first;
    in >> second;
    if(zeroth != opt::kmLen || first != opt::sBits || second != opt::rBits){
        std::cerr << "Error: Inconsistent kmer length, sketch sizes or sampling rate\n";
        exit(EXIT_FAILURE);
    }
...
//Rquires same number of samples for each sketch (assuming 1 currently)... could switch around and do the averaging first to resolve this.
void compEstSum(const uint16_t *t_Counter, size_t m, size_t n, double &F0Mean, double fMean[]) {
    unsigned p[opt::nSamp][65536];
    for(size_t i=0; i<opt::nSamp; i++)
        for(size_t j=0; j<65536; j++)
            p[i][j]=0;
    ...
    for(size_t i=0; i<opt::nSamp; i++) // nSamp in this case should always be 1
        for(size_t j=0; j<opt::rBuck; j++){
            if(t_Counter[m*opt::rBuck+j] + t_Counter[n*opt::rBuck+j] > 65535){ //not clear that this is the best way to handle edge cases
                ++p[i][65535];
            }
            else{
...
void writeSketch(std::ofstream sketchFiles[], const uint16_t *t_Counter){
    sketchFiles[0] << opt::kmLen << " ";
    sketchFiles[0] << opt::sBits << " ";
    sketchFiles[0] << opt::rBits << " ";
    for(size_t j = 0; j < opt::rBuck; j++){
        sketchFiles[0] << t_Counter[j] << " "; 
    }    
}


void outForSaveSketch(string s, const uint16_t *t_Counter){
    std::ofstream sketchFiles[1]; //will store sketche rather than hist
    std::stringstream hstm;
    string o;
    std::string::size_type p = s.find('.'); 
    o = (p != std::string::npos) ? s.substr(0,p) : s;
    hstm << o << ".ntsketch";
    sketchFiles[0].open((hstm.str()).c_str());            
    writeSketch(sketchFiles, t_Counter); //should always start at 0 when we are just writing sketches
    sketchFiles[0].close();     
}

void outForPWDistances(const uint16_t *t_Counter, unsigned fileSize){
    std::ofstream PWDistances;
    std::stringstream pwdstm;
    std::vector<double> dotProds (fileSize);
    pwdstm << opt::prefix << "_pwd" << ".txt";  
    PWDistances.open((pwdstm.str()).c_str());
    ...
    for(unsigned k=0; k<fileSize; k++) {
        ...
        double F0Mean=0.0;
        double fMean[65536];
        compEst(t_Counter+k*opt::rBuck, F0Mean, fMean);
        dotProds.at(k) = sumOfSquares(fMean);
    }

    for(unsigned m=0; m < fileSize; m++){
        double z1 = dotProds.at(m);
        double sz1 = sqrt(z1);
        for(unsigned n = m+1; n < fileSize; n++){
            double z2 = dotProds.at(n);
            double F0Mean=0.0;
...
int main(int argc, char** argv) {
    ...
    for (int c; (c = getopt_long(argc, argv, shortopts, longopts, NULL)) != -1;) {
        ...
        case 'S':
            ...
            break;
        case 'o':
            if(opt::sketch || opt::pairwise_d){
                std::cerr << "Error, arguments (sketch or pwd) and (output) considered mutually exclusive.";
                exit(EXIT_FAILURE);
            }
            arg >> opt::output;
            break;
    ...
    if (opt::prefix.empty() && opt::output.empty() && !opt::sketch && !opt::pairwise_d) {
        std::cerr << PROGRAM ": missing argument -p/-o/--sketch/-d ... \n";
        die = true;
    }
    ...
    if(totalSize<50000000000 && opt::sBits > 7) opt::sBits=7;
    ...
    if(opt::sketch || opt::pairwise_d){
        if(opt::sketch){
            ...
            for (unsigned file_i = 0; file_i < inFiles.size(); ++file_i) {
                ...
                size_t totKmer[kList.size()]; //kList.size() should always be size one, but...
                for(unsigned k=0; k<kList.size(); k++) totKmer[k]=0;
                std::ifstream in(inFiles[file_i].c_str());
                std::string samSeq;
                unsigned ftype = getftype(in,samSeq);
                if(ftype==0)
                    getEfq(in, kList, t_Counter, totKmer, 0);
                else if (ftype==1)
                    getEfa(in, kList, t_Counter, totKmer, 0);
                else if (ftype==2)
                    getEsm(in, kList, samSeq, t_Counter, totKmer, 0);
                else {
                ...
                outForSaveSketch(inFiles.at(file_i),t_Counter);
                delete [] t_Counter;
            }
        }
        if(opt::pairwise_d){
            uint16_t *t_Counter = new uint16_t [opt::rBuck*inFiles.size()];
            std::string o;
            for(unsigned file_i = 0; file_i < inFiles.size(); ++file_i){
                size_t totKmer[kList.size()];
                for(unsigned k=0; k<kList.size(); k++) totKmer[k]=0; //in this particular scenario k = 0 only... 
                std::ifstream in(inFiles[file_i].c_str());
                std::string samSeq;
                std::string::size_type p = inFiles[file_i].find('.'); 
                o = (p != std::string::npos) ? inFiles[file_i].substr(p+1) : inFiles[file_i];
                if(o == "ntsketch"){
                    readSketch(in, (t_Counter + opt::rBuck*file_i));
                }
                else{

                    unsigned ftype = getftype(in,samSeq);
                    if(ftype==0)
                        getEfq(in, kList, t_Counter, totKmer,file_i);
                    else if (ftype==1)
                        getEfa(in, kList, t_Counter, totKmer, file_i);
                    else if (ftype==2)
                        getEsm(in, kList, samSeq, t_Counter, totKmer, file_i);
                    else {
                        std::cerr << "Error in reading file: " << inFiles[file_i] << std::endl;
                        exit(EXIT_FAILURE);
                    }
                ...
                for(unsigned k=0; k<kList.size(); k++) //just trying to get rid of errors
                    #pragma omp atomic
                    totalKmers[k]+=totKmer[k];
            }
            outForPWDistances(t_Counter, inFiles.size());
            delete [] t_Counter;
        }
        return 0;
    ...
    for (unsigned file_i = 0; file_i < inFiles.size(); ++file_i) {
        ...
        unsigned ftype = getftype(in,samSeq);
        if(ftype==0)
            getEfq(in, kList, t_Counter, totKmer,0); //this is the typical case...
        else if (ftype==1)
            getEfa(in, kList, t_Counter, totKmer,0);
        else if (ftype==2)
            getEsm(in, kList, samSeq, t_Counter, totKmer,0);
